...
class _PeopleViewState extends State<PeopleView> {
  ...
  Widget build(BuildContext context) {
    ...
    final friendCount = others.where((p) => app.findFriendship(p.id)?.status == 'accepted').length;
    ...
    return VPanel(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Wrap(spacing: 10, runSpacing: 10, children: [
        _statChip('All', '${others.length}', VoxoraColors.cyan, _filter == 'all', () => setState(() => _filter = 'all')),
        _statChip('Friends', '$friendCount', VoxoraColors.success, _filter == 'friends', () => setState(() => _filter = 'friends')),
        if (pendingCount > 0)
          _statChip('Requests', '$pendingCount', VoxoraColors.lime, _filter == 'pending', () => setState(() => _filter = 'pending')),
      ]),
      ...
      Container(
        ...
        child: Row(children: [
          const Padding(
            ...
            child: Icon(Icons.search, size: 18, color: VoxoraColors.muted),
          ),
      ...
      if (filteredPeople.isEmpty)
        const Padding(
          ...
          child: Center(child: Text('No people found.', style: TextStyle(color: VoxoraColors.muted))),
        )
      else
        LayoutBuilder(builder: (context, constraints) {
          final cols = (constraints.maxWidth / 280).floor().clamp(1, 4);
          return Wrap(spacing: 14, runSpacing: 14, children: filteredPeople.map((person) {
                ...
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  ...
                  Padding(padding: const EdgeInsets.fromLTRB(16, 0, 16, 16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Transform.translate(
                      ...
                      child: VAvatar(url: person.avatarUrl, size: 52, border: true, showOnline: accepted),
                    ),
                    Transform.translate(offset: const Offset(0, -12), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Row(children: [
                        Expanded(child: Text(person.fullName, style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 15, color: VoxoraColors.text))),
                        if (accepted) const VStatusBadge(label: 'Friend', color: VoxoraColors.success),
                      ]),
                      Text('@${person.handle}', style: Theme.of(context).textTheme.bodySmall),
                      const SizedBox(height: 8),
                      Text(
                        person.bio.isEmpty ? 'No bio yet.' : person.bio,
                        style: const TextStyle(color: VoxoraColors.muted, height: 1.55, fontSize: 13),
                        maxLines: 2,
                        overflow: TextOverflow.ellipsis,
                      ),
                      if (person.interests.isNotEmpty) ...[
                        const SizedBox(height: 10),
                        Wrap(spacing: 6, runSpacing: 6, children: person.interests.take(3).map((i) => Container(
                          padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
                          decoration: BoxDecoration(
                            borderRadius: BorderRadius.circular(12),
                            color: VoxoraColors.cyan.withValues(alpha: 0.1),
                            border: Border.all(color: VoxoraColors.cyan.withValues(alpha: 0.2)),
                          ),
                          child: Text(i, style: const TextStyle(fontSize: 11, color: VoxoraColors.cyan)),
                        )).toList()),
                      ],
                      ...
                      Wrap(spacing: 8, runSpacing: 8, children: [
                        ...
                        if (incoming && friendship != null)
                          ...
                          VSecondaryButton(
                            label: accepted ? 'Friends' : (friendship != null ? 'Pending' : 'Add'),
                            icon: accepted ? Icons.check_circle : Icons.person_add_outlined,
  ...
  Widget _statChip(String label, String count, Color color, bool active, VoidCallback onTap) {
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          ...
          color: active ? color.withValues(alpha: 0.15) : VoxoraColors.surfaceLight,
          border: Border.all(color: active ? color.withValues(alpha: 0.4) : VoxoraColors.line),
        ),
        child: Row(mainAxisSize: MainAxisSize.min, children: [
          Text(label, style: TextStyle(fontWeight: FontWeight.w700, fontSize: 13, color: active ? color : VoxoraColors.muted)),
  ...
  Widget _gradientSmall(String label, IconData icon, VoidCallback onTap) {
    return GestureDetector(onTap: onTap, child: Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [BoxShadow(color: VoxoraColors.primary.withValues(alpha: 0.3), blurRadius: 8)],
      ),
      child: Row(mainAxisSize: MainAxisSize.min, children: [
