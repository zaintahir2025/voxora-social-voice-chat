...
class _MessagesViewState extends State<MessagesView> {
    ...
    return VPanel(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          if (_search.isNotEmpty)
            GestureDetector(
              ...
              child: const Padding(
                ...
                child: Icon(Icons.close, size: 16, color: VoxoraColors.muted),
              ),
            ),
        ]),
      ),
      ...
      if (_search.isNotEmpty)
          ...
          child: VListRow(
            ...
            child: Row(children: [
              ...
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text(p.fullName, style: const TextStyle(fontWeight: FontWeight.w700, color: VoxoraColors.text)),
                Text('@${p.handle}', style: Theme.of(context).textTheme.bodySmall),
              ])),
              const Icon(Icons.chat_bubble_outline, size: 16, color: VoxoraColors.muted),
            ]),
          ),
        ))
        ...
        const Padding(
          ...
          child: Center(child: Text('No conversations yet.\nSearch for someone to start chatting.', textAlign: TextAlign.center, style: TextStyle(color: VoxoraColors.muted, fontSize: 13))),
        )
      else
          ...
          return Padding(
            ...
            child: VListRow(
              ...
              child: Row(children: [
                ...
                const SizedBox(width: 12),
                Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Text(conv.title, style: const TextStyle(fontWeight: FontWeight.w700, color: VoxoraColors.text)),
  ...
  Widget _groupComposer(AppProvider app) {
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text('Create Group Chat', style: Theme.of(context).textTheme.titleMedium),
        const SizedBox(height: 10),
        if (selected.isEmpty)
          Text('Select at least 2 people.', style: Theme.of(context).textTheme.bodySmall)
        else
          Wrap(
            spacing: 6,
            runSpacing: 6,
            children: selected.map((person) => Container(
              padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 6),
              decoration: BoxDecoration(
                borderRadius: BorderRadius.circular(20),
                color: VoxoraColors.primary.withValues(alpha: 0.15),
                border: Border.all(color: VoxoraColors.primary.withValues(alpha: 0.3)),
              ),
              child: Row(mainAxisSize: MainAxisSize.min, children: [
                Text(person.fullName, style: const TextStyle(fontSize: 12, color: VoxoraColors.text, fontWeight: FontWeight.w600)),
                const SizedBox(width: 4),
                GestureDetector(
                  onTap: () => setState(() => _groupMemberIds.remove(person.id)),
                  child: const Icon(Icons.close, size: 14, color: VoxoraColors.primary),
                ),
              ]),
            )).toList(),
          ),
              ...
              return GestureDetector(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    color: checked ? VoxoraColors.primary.withValues(alpha: 0.1) : VoxoraColors.surface,
                    border: Border.all(color: checked ? VoxoraColors.primary.withValues(alpha: 0.3) : VoxoraColors.line),
                  ),
                  child: Row(children: [
                    VAvatar(url: person.avatarUrl, size: 32),
                    const SizedBox(width: 10),
                    Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Text(person.fullName, style: const TextStyle(fontWeight: FontWeight.w700, color: VoxoraColors.text, fontSize: 13)),
                      Text('@${person.handle}', style: const TextStyle(fontSize: 11, color: VoxoraColors.muted)),
                    ])),
                    Icon(
                      checked ? Icons.check_circle : Icons.circle_outlined,
                      size: 20,
                      color: checked ? VoxoraColors.primary : VoxoraColors.muted,
                    ),
                  ]),
                ),
              );
        ...
        TextField(
          ...
          decoration: const InputDecoration(hintText: 'First message (optional)', prefixIcon: Icon(Icons.message, size: 18, color: VoxoraColors.muted)),
        ),
        const SizedBox(height: 12),
        VGradientButton(
          label: 'Create Group',
  ...
  Widget _panel(AppProvider app) {
    ...
    if (conv == null) {
      return VPanel(child: const VEmptyState(icon: Icons.chat_bubble_outline, title: 'No conversation', body: 'Search for someone to start chatting.'));
    }
      ...
      child: SizedBox(
        ...
        child: Column(children: [
          ...
          Container(
            ...
            child: Row(children: [
              ...
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                Row(children: [
                  const VPulseDot(color: VoxoraColors.online, size: 6),
          ...
          Container(
            ...
            decoration: const BoxDecoration(border: Border(top: BorderSide(color: VoxoraColors.line))),
            child: Row(children: [
              Expanded(child: TextField(
                controller: _msgC,
                onChanged: (_) => setState(() {}),
